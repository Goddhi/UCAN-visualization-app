...
export interface UCANNodeData {
  ...
  capabilities: string[];
  expiration?: string;
  // FIX: Add this line so the recursive tree works
  proofs?: UCANNodeData[]; 
}
    ...
    >
      ...
      <div className="flex items-start gap-2 mb-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {/* Safety check for ID slicing */}
          <div className="text-sm font-mono text-text-primary truncate">
            {data.id ? data.id.slice(0, 12) : "Unknown"}...
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div>
          ...
          <div className="text-xs font-mono text-text-secondary truncate">{data.issuer}</div>
        </div>
        <div>
          <div className="text-xs text-text-tertiary mb-1">Audience</div>
          <div className="text-xs font-mono text-text-secondary truncate">{data.audience}</div>
        </div>
        <div>
          <div className="text-xs text-text-tertiary mb-1">Capabilities</div>
          <div className="flex flex-wrap gap-1">
            {data.capabilities.map((cap, idx) => (
              <span key={idx} className="px-2 py-0.5 bg-accent-primary/10 text-accent-primary text-xs rounded">
                {cap}
              </span>
            ))}
          </div>
