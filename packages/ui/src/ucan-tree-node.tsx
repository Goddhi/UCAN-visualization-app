...
interface UCANTreeNodeProps {
  node: UCANNodeData;
  onNodeClick?: (node: UCANNodeData) => void;
}

export const UCANTreeNode = ({ node, onNodeClick }: UCANTreeNodeProps) => {
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-sm font-mono text-text-primary truncate max-w-[180px]">
                {node.id.slice(0, 12)}...
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-8 pt-4">
            {node.proofs?.map((proof, idx) => (
              <div key={proof.id} className="relative">
                ...
                <UCANTreeNode node={proof} onNodeClick={onNodeClick} />
              </div>
            ))}
          </div>
        </div>
